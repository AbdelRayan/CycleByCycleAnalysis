function varargout = wcoherGenzelVersion(s1,s2,scales,wname,varargin)
...
SamplingFreq = 1250;
flag_SMOOTH = true;
[cfs_s1, Ft] = cwt(s1, wname, SamplingFreq, "FrequencyLimits", [20 180]);
cfs_s10   = cfs_s1;
cfs_s1    = smoothCFSAbdel(abs(cfs_s1).^2,flag_SMOOTH,NSW,NTW);
cfs_s1    = sqrt(cfs_s1);
cfs_s2    = cwt(s2,wname,SamplingFreq, "FrequencyLimits", [20 180]);
cfs_s20   = cfs_s2;
cfs_s2    = smoothCFSAbdel(abs(cfs_s2).^2,flag_SMOOTH,NSW,NTW);
cfs_s2    = sqrt(cfs_s2);
cfs_cross = conj(cfs_s10).*cfs_s20;
cfs_cross = smoothCFSAbdel(cfs_cross,flag_SMOOTH,NSW,NTW);
